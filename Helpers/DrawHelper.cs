...
namespace StarlightRiver.Helpers
{
    ...
    public static partial class Helper
    {
        ...
        public static void DrawWithLighting(Rectangle pos, Texture2D tex, Rectangle source, Color color = default) //just going to make this an overload for now. TODO: optional param conversion?
        {
            if (Main.dedServ || !OnScreen(new Rectangle(pos.X, pos.Y, tex.Width, tex.Height))) return;
            ...
            verticies[0] = new VertexPositionTexture(new Vector3(ConvertX(pos.X), ConvertY(pos.Y), 0), source.TopLeft() / tex.Size());
            verticies[1] = new VertexPositionTexture(new Vector3(ConvertX(pos.X + pos.Width), ConvertY(pos.Y), 0), source.TopLeft() / tex.Size() + Vector2.UnitX * source.Width / tex.Width);
            verticies[2] = new VertexPositionTexture(new Vector3(ConvertX(pos.X), ConvertY(pos.Y + pos.Height), 0), source.TopLeft() / tex.Size() + Vector2.UnitY * source.Height / tex.Height);

            verticies[3] = new VertexPositionTexture(new Vector3(ConvertX(pos.X + pos.Width), ConvertY(pos.Y), 0), source.TopLeft() / tex.Size() + Vector2.UnitX * source.Width / tex.Width);
            verticies[4] = new VertexPositionTexture(new Vector3(ConvertX(pos.X + pos.Width), ConvertY(pos.Y + pos.Height), 0), source.BottomRight() / tex.Size());
            verticies[5] = new VertexPositionTexture(new Vector3(ConvertX(pos.X), ConvertY(pos.Y + pos.Height), 0), source.TopLeft() / tex.Size() + Vector2.UnitY * source.Height / tex.Height);
