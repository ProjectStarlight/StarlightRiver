...
float4 BellShader(float4 unused : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        float distanceToPixel =  length(diffPosition * float2(1.5, 1.0));
        float radius = 180.0 + progresses[i] * 1000.0;

        float modifiedDistance = -(distanceToPixel - radius);

        float waveIntensity = min(5.0, abs(50.0 / modifiedDistance)) * uIntensity;
        ...
        textureColor.rgb = lerp(textureColor.rgb, color * textureColor.a, uOpacity);
