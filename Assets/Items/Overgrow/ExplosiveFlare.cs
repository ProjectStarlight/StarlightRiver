...
namespace StarlightRiver.Projectiles.WeaponProjectiles
{
    internal class ExplosiveFlare : ModProjectile
    {
		...
		private void Explode(NPC target)
        {
			...
			{
				Projectile.NewProjectile(projectile.position + (Vector2.UnitY.RotatedBy(projectile.rotation + 3.14) * 15), Vector2.UnitY.RotatedBy(projectile.rotation + i) * 15, ModContent.ProjectileType<FlareExplosion2>(), projectile.damage, 0, projectile.owner);
				for (int j = 0; j < 25; j++)
                {
    ...
    internal class FlareExplosion2 : ModProjectile
    {
		public override void SetDefaults()
		{
			projectile.width = 24;
