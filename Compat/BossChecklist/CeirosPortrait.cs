	...
	class CeirosPortrait
	{
		...
		public static void DrawCeirosPortrait(SpriteBatch spriteBatch, Rectangle rect, Color color)
		{
			if (Main.rand.NextBool(2))
				ceirosSystem.AddParticle(rect.Center() + Vector2.One.RotatedByRandom(6.28f) * Main.rand.NextFloat(100), Vector2.UnitY * -3, 0, Main.rand.NextFloat(0.3f), new Color(200, 200, 0), 100, Vector2.Zero);

			// have to call this in draw unfortunately.
			ceirosSystem.UpdateParticles();
