...
namespace StarlightRiver.Core.VerletGenerators
{
	class RectangularBanner : VerletChain
	{
		...
		public override void PrepareStrip(float scale)
		{
			...
			verticies[0] = new VertexPositionColor((ropeSegments[0].ScreenPos + Vector2.UnitY.RotatedBy(rotation - Math.PI / 4) * -scale).ToVector3().ToScreenspaceCoord(), ropeSegments[0].color);
			verticies[1] = new VertexPositionColor((ropeSegments[0].ScreenPos + Vector2.UnitY.RotatedBy(rotation + Math.PI / 4) * -scale).ToVector3().ToScreenspaceCoord(), ropeSegments[0].color);
			...
			for (int k = 1; k < segmentCount - 1; k++)
			{
				...
				verticies[point] = new VertexPositionColor((ropeSegments[k].ScreenPos + Vector2.UnitY.RotatedBy(rotation2 - Math.PI / 4) * -scale).ToVector3().ToScreenspaceCoord(), ropeSegments[k].color);
				verticies[point + 1] = new VertexPositionColor((ropeSegments[k].ScreenPos + Vector2.UnitY.RotatedBy(rotation2 + Math.PI / 4) * -scale).ToVector3().ToScreenspaceCoord(), ropeSegments[k].color);
