...
namespace StarlightRiver.Core.Systems
{
	public class CameraSystem : ModSystem
	{
        ...
        private static MoveModifier MoveModifier = new MoveModifier();
		...
		{
            if (Shake > 120 * ModContent.GetInstance<Configs.GraphicsConfig>().ScreenshakeMult) //clamp screenshake to (120 * config) to prevent utter chaos
