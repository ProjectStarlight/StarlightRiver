...
namespace StarlightRiver.Core.MistSystem
{
	public class MistField : IDisposable
	{
		...
		public MistField(int SimulationSize, int CellSize, Vector2 GlobalSpace)
		{
			...
			var c = new Color[DensityTarget.Width * DensityTarget.Height];
			var cV = new Color[DensityTarget.Width * DensityTarget.Height];
			var cV2 = new Color[DensityTarget.Width * DensityTarget.Height];
			...
			var m = Matrix.CreateOrthographicOffCenter(0, N, N, 0, 0, -1);
			m.M41 += -0.5f * m.M11;
			m.M42 += -0.5f * m.M22;
		...
		public void Update()
		{
			...
			AdSource(DensityTarget, AddVecSourceBuffer, (sb) => sb.Draw(magicPixel, new Rectangle(N / 2 + 20, N / 2 - 20, 4, 4), new Color(0f, 1f, 0)));

			AdSource(VectorTargetX, AddVecSourceBuffer, (sb) => sb.Draw(magicPixel, new Rectangle(N / 2 + 20, N / 2 - 20, 8, 8), ConvertToColor(v.X)));

			AdSource(VectorTargetY, AddVecSourceBuffer, (sb) => sb.Draw(magicPixel, new Rectangle(N / 2 + 20, N / 2 - 20, 8, 8), ConvertToColor(v.Y)));
		...
		public void Project(RenderTarget2D u, RenderTarget2D v, RenderTarget2D p, RenderTarget2D div)
		{
			...
			{
				var rect = new Rectangle(2, 2, p.Bounds.X - 4, p.Bounds.Y - 4);
