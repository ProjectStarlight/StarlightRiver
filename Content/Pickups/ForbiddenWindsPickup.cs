...
namespace StarlightRiver.Content.Pickups
{
	internal class ForbiddenWindsPickup : AbilityPickup, IDrawPrimitive
	{
		...
		public override void PickupVisuals(int timer)
		{
			...
			{
				string message = Language.GetTextValue("Mods.StarlightRiver.Custom.UI.TextCard.ForbiddenWindsPickupMessage", StarlightRiver.Instance.AbilityKeys.Get<Dash>().GetAssignedKeys().Count > 0 ?
					StarlightRiver.Instance.AbilityKeys.Get<Dash>().GetAssignedKeys()[0] :
					Language.GetTextValue("Mods.StarlightRiver.Custom.UI.TextCard.PleaseBindAKey"));

				Main.LocalPlayer.GetHandler().GetAbility<Dash>(out Dash dash);
				UILoader.GetUIState<TextCard>().Display(Language.GetTextValue("Mods.StarlightRiver.Custom.UI.TextCard.ForbiddenWindsPickupTitle"), message, dash);
