...
namespace StarlightRiver.Content.Pickups
{
	class InfusionSlotPickup : AbilityPickup
	{
		...
		public override void PickupVisuals(int timer)
		{
			...
			if (timer == 559)
			{
				UILoader.GetUIState<TextCard>().Display(Language.GetTextValue("Mods.StarlightRiver.Custom.UI.TextCard.MysteriousTechnologyTitle"), Language.GetTextValue("Mods.StarlightRiver.Custom.UI.TextCard.MysteriousTechnologyMessage"), time: 360);
