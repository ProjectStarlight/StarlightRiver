...
namespace StarlightRiver.Content.Pickups
{
	internal class CodexPickup : AbilityPickup
	{
		...
		public override void PickupVisuals(int timer)
		{
			...
			if (timer == 119)
			{
				string message = Language.GetTextValue("Mods.StarlightRiver.Custom.UI.TextCard.StarlightCodexMessage");

				UILoader.GetUIState<TextCard>().Display(Language.GetTextValue("Mods.StarlightRiver.Custom.UI.TextCard.StarlightCodexTitle"), message, null, 240);
