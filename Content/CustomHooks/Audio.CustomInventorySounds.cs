	...
	class CustomInventorySounds : HookGroup
	{
		...
		private SlotId PlayNewSound(int originalSoundID, Item Item)
		{
			...
			if (Item.ModItem is ICustomInventorySound)
				...
				return SoundHelper.PlayPitched("Pickups/PickupPotion", 1, 0.5f + pitch * 0.5f);
				...
				return SoundHelper.PlayPitched(SoundID.Coins, 1, 0.3f + pitch);

			else if (Item.dye > 0) //dyes
				return SoundHelper.PlayPitched("Pickups/PickupPotion", 1, 0.9f + pitch * 0.25f);

			else if (Item.createTile != -1) //placables
				return SoundHelper.PlayPitched("Pickups/PickupGeneric", 1, 1 + pitch);

			else if (Item.defense > 0) //armor and shields
				return SoundHelper.PlayPitched("Pickups/PickupArmor", 1, 0.5f + pitch);

			else if (Item.vanity) //vanity
				return SoundHelper.PlayPitched("Pickups/PickupVanity", 1, 0.5f + pitch);

			else if (Item.accessory) //non-vanity non-shield accessories
				return SoundHelper.PlayPitched("Pickups/PickupAccessory", 1, 0.1f + pitch);

			else if (Item.pick > 0 || Item.axe > 0 || Item.hammer > 0) //tools
				return SoundHelper.PlayPitched("Pickups/PickupTool", 1, 0.5f + pitch);

			else if (Item.damage > 0) //weapons and ammo
			{
				if (Item.DamageType.Type == Terraria.ModLoader.DamageClass.Melee.Type) //melee weapons
					return SoundHelper.PlayPitched("Pickups/PickupMelee", 1, 0.5f + pitch);

				else if (Item.useAmmo == ItemID.MusketBall) //guns
					return SoundHelper.PlayPitched("Pickups/PickupGun", 1, 0 + pitch);

				else if (Item.DamageType.Type == Terraria.ModLoader.DamageClass.Ranged.Type) //other ranged weapons
					return SoundHelper.PlayPitched("Pickups/PickupGeneric", 1, 0.9f + pitch * 0.5f);
					...
					return SoundHelper.PlayPitched("Pickups/PickupGeneric", 1, 0.5f + pitch);
