...
namespace StarlightRiver.Content.CustomHooks
{
	class DrawForegrounds : HookGroup
	{
		...
		public void DrawForeground(On.Terraria.Main.orig_DrawInterface orig, Main self, GameTime gameTime)
		{
			...
			foreach (Foregrounds.Foreground fg in ForegroundLoader.Foregrounds) //TODO: Perhaps create some sort of ActiveForeground list later? especially since we iterate twice for the over UI ones
			{
			...
			foreach (Foregrounds.Foreground fg in ForegroundLoader.Foregrounds)
			{
				if (fg != null && fg.OverUI)
					fg.Render(Main.spriteBatch);
			}
