	...
	class AshHellBackground : HookGroup
    {
        ...
        private void DrawAltBackground(On.Terraria.Main.orig_DrawUnderworldBackground orig, Main self, bool flat)
        {
            ...
            if (Main.LocalPlayer.GetModPlayer<BiomeHandler>().ZoneAshhell)
            {
                ...
                for (int i = 3; i >= 0; i--)
                {
                    Texture2D tex = ModContent.Request<Texture2D>("StarlightRiver/Assets/Backgrounds/AshHell" + i).Value;
                    ...
                    vector2 *= num3;
                    if (flat) zero.Y += (ModContent.Request<Texture2D>("StarlightRiver/Assets/Backgrounds/AshHell0").Value.Height >> 1) * 1.3f - vector2.Y;
