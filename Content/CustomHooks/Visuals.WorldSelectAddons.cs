...
namespace StarlightRiver.Content.CustomHooks
{
	...
	{
		//Disabled for now due to performance issues and having no use.
		...
		readonly Dictionary<UIWorldListItem, TagCompound> worldDataCache = new();
		...
		private void VoidIcon(On.Terraria.GameContent.UI.Elements.UIWorldListItem.orig_DrawSelf orig, UIWorldListItem self, SpriteBatch spriteBatch)
		{
			...
			if (worldDataCache.TryGetValue(self, out TagCompound tag3) && tag3 != null)
			...
			var rect = new Rectangle((int)pos.X - 122, (int)pos.Y + 6, tex.Width, tex.Height);
