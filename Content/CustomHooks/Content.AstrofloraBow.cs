	...
	public class AstrofloraBow : HookGroup
	{
		...
		public override void Load()
		{
			...
			On.Terraria.Main.DrawNPCs += (orig, self, behindTiles) =>
			{
				...
				var crosshairColor = Color.Lerp(Color.White, Main.cursorColor, (float)(Math.Sin(Main.timeForVisualEffects * 10) + 1) / 2);

				Vector2 sizeOfDonut = Vector2.One * 32;

				var drawInfo = new List<(NPC, float)>();
				...
				foreach ((NPC, float) info in drawInfo)
				{
					...
					var destinationRectangle = new Rectangle((int)aboveNpcTop.X, (int)aboveNpcTop.Y, (int)sizeOfDonut.X, (int)sizeOfDonut.Y);
