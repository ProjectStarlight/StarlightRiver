...
namespace StarlightRiver.Content.Foregrounds
{
	class AshHellForeground : ParticleForeground
	{
		...
		private void UpdateAshParticles(Particle particle)
		{
			...
			Color color;
			if (progress > 0.7f)
				color = Color.Lerp(Color.White, Color.Orange, 1 - (progress - 0.7f) / 0.3f);
			else if (progress > 0.5f)
				color = Color.Lerp(Color.Orange, Color.DarkRed, 1 - (progress - 0.5f) / 0.2f);
			else if (progress > 0.4f)
				color = Color.Lerp(Color.DarkRed, Color.Gray, 1 - (progress - 0.4f) / 0.1f);
			else
				color = Color.Gray;
