	...
	class TemplePick : ModItem
	{
		...
		public override void UpdateInventory(Player Player) //strange hook to be doing this in but it seemeed the best solution at the time.
		{
			if (Player.HeldItem == Item) //bleghhh
			{
				...
				{
					var d = Dust.NewDustPerfect(Player.Center, DustType<Content.Dusts.PickCharge>(), Vector2.UnitY.RotatedBy(Charge / 120f * 6.28f) * 30, 0, Color.LightYellow, 2);
			...
			if (!Main.mouseRight && Charge > 0 && !Whirling || Player.HeldItem != Item)
				Charge = 0;
