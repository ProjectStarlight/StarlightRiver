		...
		private void UpdateMinions(Player player)
		{
			foreach (Projectile proj in Main.projectile.Where(n => n.active && n.type == ModContent.ProjectileType<FleshApparation>() && n.owner == player.whoAmI))
			{
				if (proj.ModProjectile is FleshApparation mp)
				{
					...
					player.lifeRegen += (int)(10 * mp.healPower);
				}
			}
		}
	}
	...
	internal class FleshApparation : ModProjectile
	{
		...
		public void Attack()
		{
			...
			if (Timer < 60) //Position away from target
			{
				...
				var diff = Vector2.Normalize(target - targetNPC.Center);
			...
			if (Timer >= 60 && Timer < 100) //slow down
			{
				...
				Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<Dusts.Cinder>(), Vector2.One.RotatedByRandom(6.28f) * 0.2f, 0, Color.Red, 0.5f + Main.rand.NextFloat(0.25f));
			}
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			for (int k = 0; k < 20; k++)
			{
				Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<Dusts.Cinder>(), Vector2.One.RotatedByRandom(6.28f) * Main.rand.NextFloat(3), 0, Color.Red, 0.5f + Main.rand.NextFloat(0.25f));
			}
