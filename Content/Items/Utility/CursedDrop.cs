	...
	class CurseDropApply : GlobalItem
	{
		...
		public override void RightClick(Item item, Player player)
		{
			if (Main.mouseItem.type == ModContent.ItemType<CursedDrop>() && ItemLoader.CanReforge(item))
			{
				...
				for (int k = 0; k <= 50; k++)
					CursedAccessorySystem.CursedSystem.AddParticle(Main.MouseScreen, Vector2.One.RotatedByRandom(6.28f) * Main.rand.NextFloat(0.75f), 0, 1, new(25, 17, 49, 0), 60, Vector2.Zero, alpha: 0);
