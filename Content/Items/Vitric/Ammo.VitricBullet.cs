...
namespace StarlightRiver.Content.Items.Vitric
{
	...
	internal class VitricBulletProjectile : ModProjectile
	{
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			{
				var proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), target.position, Vector2.Zero, ModContent.ProjectileType<VitricBulletCrystalProjectile>(), Projectile.damage, 0f, Projectile.owner);
	...
	internal class VitricBulletCrystalExplosion : ModProjectile
	{
		...
		{
			return target.whoAmI == NPCWhoAmI; //only hit the npc that the crystal was on
		}
