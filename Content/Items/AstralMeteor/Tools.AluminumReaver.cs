...
namespace StarlightRiver.Content.Items.AstralMeteor
{
        ...
        public override bool Shoot(Player Player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (Main.rand.Next(10) == 0)
                for (int k = 0; k < Main.maxNPCs; k++)
                {
                    ...
                    {
                        Projectile.NewProjectile(source,  target.Center, Vector2.Zero, ProjectileType<ReaverLightningNode>(), damage, 0, 0, 2, 100);
    ...
    internal class ReaverLightningNode : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            {
                Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), chosenTarget.Center, Vector2.Zero, ProjectileType<ReaverLightningNode>(), damage, knockback, Projectile.owner, Projectile.ai[0] - 1, Projectile.ai[1]);
