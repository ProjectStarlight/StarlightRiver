...
namespace StarlightRiver.Content.Items.SteampunkSet
{
	...
	public class BuzzsawProj : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor) //extremely messy code I ripped from a weapon i made for spirit :trollge:
		{
			...
			var origin = new Vector2(texture.Width / 2f, height1 / 2f);
			Vector2 position = (Projectile.position - 0.5f * (direction * OFFSET) + new Vector2(Projectile.width, Projectile.height) / 2f + Vector2.UnitY * Projectile.gfxOffY - Main.screenPosition).Floor();
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			...
			if (Main.myPlayer == Projectile.owner)
			{
				...
				if (Helper.IsFleshy(target))
				{
					...
					var proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), target.Center, Vector2.Zero, bloodID, 0, 0, Projectile.owner);
