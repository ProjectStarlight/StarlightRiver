...
namespace StarlightRiver.Content.Items.SteampunkSet
{
    public class JetwelderGatler : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            for (int i = 1; i < 9; i++)
            {
                Gore.NewGore(Projectile.GetSource_FromThis(), Projectile.GetSource_FromThis(), Projectile.Center + Main.rand.NextVector2Circular(Projectile.width / 2, Projectile.height / 2), Main.rand.NextVector2Circular(5, 5), Mod.Find<ModGore>("JetwelderGatler_Gore" + i.ToString()).Type, 1f); //PORTTODO: Fix default gores
            }
        }
        ...
        private void FireBullets()
        {
            if ((gunFrame == 0 || gunFrame == 4) && Projectile.frameCounter % 2 == 0)
            {
                ...
                Gore.NewGore(Projectile.GetSource_FromThis(),Projectile.GetSource_FromThis(), Projectile.Center, new Vector2(Math.Sign(dir.X) * -1, -0.5f) * 2, Mod.Find<ModGore>("JetwelderCasing").Type, 1f); 
