...
namespace StarlightRiver.Items.Armor
{
    public static class ArmorHelper
    {
        ...
        public static void QuickDrawHelmet(PlayerDrawInfo info, string texture, Color color, float scale, Vector2 offset)
        {
            ...
            Main.playerDrawData.Add(new DrawData(tex, (info.position - Main.screenPosition + offset).ToPoint16().ToVector2(), null, color * ((255 - info.drawPlayer.immuneAlpha) / 255f), info.drawPlayer.headRotation, tex.Size() / 2, scale, info.spriteEffects, 0));
        }
    }
}
