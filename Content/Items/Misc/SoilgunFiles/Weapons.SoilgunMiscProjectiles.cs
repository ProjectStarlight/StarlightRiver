...
namespace StarlightRiver.Content.Items.Misc.SoilgunFiles
{
	...
	class SoilgunSandGrain : ModProjectile
	{
		...
		{
			return lightColor * (Projectile.scale < 0.5f ? Projectile.scale * 2 : 1);
		}
	...
	class SoilgunLifeSteal : ModProjectile, IDrawPrimitive
	{
		...
		public override void AI()
		{
			...
			{
				var dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.LifeDrain, 0f, 0f, 25, default, Main.rand.NextFloat(0.9f, 1.1f));
				dust.noGravity = true;
			}
		...
		{
			trail ??= new Trail(Main.instance.GraphicsDevice, 13, new TriangularTip(4), factor => 4, factor => Color.Red * 0.8f * factor.X);
	...
	internal class SoilgunCoinsProjectile : ModProjectile
	{
			...
			if (npc != null && Collision.CanHit(Projectile.Center, 1, 1, npc.Center, 1, 1) && !npc.dontTakeDamage && !npc.immortal && !foundTarget && Projectile.timeLeft < 90)
			{
				...
				if (!HasCharged)
				{
					...
					for (int i = 0; i < Repeats; ++i)
					{
						...
						var dust3 = Dust.NewDustPerfect(Projectile.Center, ChooseDustType(), null, 0, default, 1.1f);
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			var frameRect = new Rectangle(0, FrameHeight * Projectile.frame, texture.Width, FrameHeight);
	...
	public class HauntedSoul : ModProjectile, IDrawPrimitive
	{
		...
		public override void Kill(int timeLeft)
		{
			...
			{
				float angle2 = 6.2831855f * i / Repeats;
				var dust3 = Dust.NewDustPerfect(Projectile.Center, DustID.Shadowflame, null, 0, default, 1.1f);
		...
		private void ManageTrail()
		{
			trail ??= new Trail(Main.instance.GraphicsDevice, 13, new TriangularTip(4), factor => 16, factor => new Color(52, 21, 141) * 0.8f * factor.X);
	...
	class SoilgunVitricCrystals : ModProjectile
	{
		...
		public override void Kill(int timeLeft)
		{
			...
			if (exploding)
			{
				...
				Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<Dusts.NeedlerDustSlowFade>(), Vector2.One.RotatedByRandom(6.18f) * Main.rand.NextFloat(3f, 4.5f), 85, default, Main.rand.NextFloat(0.5f, 0.8f)).rotation = Main.rand.NextFloat(6.18f);

				Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<Dusts.NeedlerDustGlow>(), Vector2.One.RotatedByRandom(6.18f) * Main.rand.NextFloat(3f, 4.5f), 85, default, Main.rand.NextFloat(0.5f, 0.8f)).rotation = Main.rand.NextFloat(6.18f);
				return;
			}
