...
namespace StarlightRiver.Content.Items.Misc
{
	internal class FiletKnife : ModItem
	{
		...
		public override void OnHitNPC(Player player, NPC target, int damage, float knockBack, bool crit)
		{
			if (crit)
			{
				...
				int itemType = -1;
				itemType = Main.rand.Next(3) switch
				{
	...
	public class FiletSlash : ModProjectile, IDrawPrimitive
	{
		...
		public override void AI()
		{
			...
			trail = new Trail(Main.instance.GraphicsDevice, 20 + widthExtra * 2, new TriangularTip((int)((target.width + target.height) * 0.6f)), factor => 10 * (1 - Math.Abs(1 - factor - Projectile.timeLeft / (float)(BASETIMELEFT + 5))) * (Projectile.timeLeft / (float)BASETIMELEFT), factor => Color.Lerp(Color.Red, Color.DarkRed, factor.X) * 0.8f)
			{
