...
namespace StarlightRiver.Content.Items.Misc
{
	class GeodeBow : ModItem
	{
		...
		public override void AddRecipes()
		{
			...
			recipe.AddIngredient(ItemType<ExoticGeodeArtifactItem>(), 5);
	...
	internal class GeodeBowProj : ModProjectile
	{
		...
		private void Shoot()
		{
			...
			var proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.rotation.ToRotationVector2() * (speed + 12), type, damage, knockBack, Owner.whoAmI);
	...
	internal class GeodeBowGrowth : ModProjectile
	{
		...
		private readonly float[] crystalScales = new float[5];
		...
		private void Shatter(Projectile proj)
		{
			...
			for (int k = 0; k < 12; k++)
			{
				Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(30, 30), DustType<Dusts.ArtifactSparkles.GeodeArtifactSparkleFast>(), Main.rand.NextVector2Circular(7, 7), 0, default, Main.rand.NextFloat(0.85f, 1.15f));
			}
			...
			Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ProjectileType<GeodeBowExplosion>(), (int)(Math.Pow(counter, 0.7f) * Projectile.damage), Projectile.knockBack, Owner.whoAmI, 0, (int)Math.Sqrt(counter) * 0.5f);

			for (int j = 0; j < 16; j++)
			{
				Vector2 direction = Main.rand.NextVector2CircularEdge(1, 1);
				var shrapnel = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center + direction * 10, direction * Main.rand.Next(5, 10) * (float)Math.Sqrt(counter), ProjectileType<GeodeBowShrapnel>(), 0, 0, Owner.whoAmI);
	...
	internal class GeodeBowExplosion : ModProjectile
	{
		...
		public override void AI()
		{
			var crystals = Main.projectile.Where(x => x.active && x.type == ProjectileType<GeodeBowGrowth>()).ToList();
	...
	public class GeodeBowShrapnel : ModProjectile, IDrawPrimitive
	{
		...
		{
			trail ??= new Trail(Main.instance.GraphicsDevice, 15, new TriangularTip(4), factor => MathHelper.Lerp(1f, 0.35f, factor) * width * MathHelper.Lerp(0.2f, 1, Projectile.timeLeft / 90.0f), factor => Color.Lerp(Color.Magenta, Color.White, Projectile.timeLeft / 90.0f));
	...
	public class GeodeBowGProj : GlobalProjectile
	{
		...
		public override void OnHitNPC(Projectile projectile, NPC target, NPC.HitInfo hit, int damageDone)
		{
			...
			var proj = Projectile.NewProjectileDirect(projectile.GetSource_FromThis(), target.Center, Vector2.Zero, ProjectileType<GeodeBowGrowth>(), projectile.damage, projectile.knockBack, projectile.owner);
