...
namespace StarlightRiver.Content.Items.Starwood
{
	public class StarwoodStaff : StarwoodItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            var newVelocity = velocity * (mp.empowered ? 1.05f : 1f);
            ...
            {
                var projVelocity = newVelocity.RotatedBy(Main.rand.NextFloat(-0.05f, 0.05f) * (k * 0.10f + 1)) * Main.rand.NextFloat(0.9f, 1.1f) * (k * 0.15f + 1);
            ...
            for (int k = 0; k < 10; k++)
            {
                var pos = staffEndPosition + new Vector2(Main.rand.NextFloat(-10f, 10f), Main.rand.NextFloat(-5f, 15f));
                var dustVelocity = (newVelocity * Main.rand.NextFloat(0.01f, 0.1f)).RotatedBy(Main.rand.NextFloat(-0.5f, 0.5f)) + player.velocity * 0.5f;
