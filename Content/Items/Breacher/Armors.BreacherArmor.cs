...
namespace StarlightRiver.Content.Items.Breacher
{
	...
	public class BreacherChest : ModItem
	{
		...
		{
			return head.type == ModContent.ItemType<BreacherHead>() && legs.type == ModContent.ItemType<BreacherLegs>();
		}
	...
	public class SpotterDrone : ModProjectile
	{
		...
		private Vector2 targetPos => Vector2.Lerp(target.Bottom, target.Top, 0.5f + (float)Math.Cos((ScanTimer - 100) * 2 * 6.28f / (ScanTime - 100)) / 2f);
		private Vector2 targetPos2 => Vector2.Lerp(target.Top, target.Bottom, 0.5f + (float)Math.Cos((ScanTimer - 100) * 2 * 6.28f / (ScanTime - 100)) / 2f);
	...
	internal class OrbitalStrike : ModProjectile, IDrawPrimitive
	{
		...
		private void Explode(NPC target)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				Dust.NewDustPerfect(Projectile.Center + new Vector2(20, 70), ModContent.DustType<BreacherDustLine>(), Vector2.One.RotatedByRandom(6.28f) * Main.rand.NextFloat(12, 26), 0, new Color(48, 242, 96), Main.rand.NextFloat(0.7f, 0.9f));
				Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<BreacherDustStar>(), Main.rand.NextFloat(6.28f).ToRotationVector2() * Main.rand.NextFloat(8), 0, new Color(48, 242, 96), Main.rand.NextFloat(0.1f, 0.2f));
			}
