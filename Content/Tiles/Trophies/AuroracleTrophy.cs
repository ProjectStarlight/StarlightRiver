...
namespace StarlightRiver.Content.Tiles.Trophies
{
    class AuroracleTrophy : ModTile
    {
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
            ...
            {
                var headBlob = Request<Texture2D>(AssetDirectory.SquidBoss + "BodyOver").Value;
                var headBlobGlow = Request<Texture2D>(AssetDirectory.SquidBoss + "BodyOverGlow").Value;
                var headBlobSpecular = Request<Texture2D>(AssetDirectory.SquidBoss + "BodyOverSpecular").Value;

                var tex2 = Request<Texture2D>(AssetDirectory.TrophyTile + Name + "Glow2").Value;

                var pos = (new Vector2(i, j) + Helpers.Helper.TileAdj) * 16 - Vector2.One * 8 - Main.screenPosition;

                float sin = 1 + (float)Math.Sin(Main.GameUpdateCount * 0.05f);
                float cos = 1 + (float)Math.Cos(Main.GameUpdateCount * 0.05f);
                float scale = 0.8f;

                var color = new Color(0.5f + cos * 0.25f, 0.8f, 0.5f + sin * 0.25f).MultiplyRGB(Lighting.GetColor(i - 1, j - 1));
