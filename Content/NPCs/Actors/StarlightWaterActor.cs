...
namespace StarlightRiver.Content.NPCs.Actors
{
	...
	public class TransformableItem : GlobalItem
	{
		...
		public override void SetDefaults(Item Item) //TOOD: Probably move this later or make it modular? pee pee poo poo
		{
			...
			if (Item.vanity)
			{
				if (Item.headSlot != -1 && Item.type != ModContent.ItemType<AncientStarwoodHat>())
					transformType = ModContent.ItemType<AncientStarwoodHat>();
				else if (Item.bodySlot != -1 && Item.type != ModContent.ItemType<AncientStarwoodChest>())
					transformType = ModContent.ItemType<AncientStarwoodChest>();
				else if (Item.legSlot != -1 && Item.type != ModContent.ItemType<AncientStarwoodBoots>())
					transformType = ModContent.ItemType<AncientStarwoodBoots>();
			}
		}
		...
		public override bool PreDrawInWorld(Item Item, SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
		{
			if (transformTime > 0)
			{
				...
				float alphaMaster = (float)Math.Sin(transformTime / 300f * 3.14f);

				float alpha = (1.0f + (float)Math.Sin(transformTime / 75f * 3.14f) * 0.5f) * alphaMaster;
				...
				float alpha2 = (1.0f + (float)Math.Sin(transformTime / 150f * 3.14f) * 0.5f) * alphaMaster;
				...
				float alpha3 = (1.0f + (float)Math.Sin(transformTime / 37.5f * 3.14f) * 0.5f) * alphaMaster;
