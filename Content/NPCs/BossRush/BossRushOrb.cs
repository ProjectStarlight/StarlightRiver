...
namespace StarlightRiver.Content.NPCs.BossRush
{
	internal class BossRushOrb : ModNPC, ILoadable, IHintable
	{
		...
		private void DrawRings(Color lightColor)
		{
			...
			Main.spriteBatch.End();
			Main.spriteBatch.Begin(default, BlendState.Additive, default, default, RasterizerState.CullNone, null, Main.GameViewMatrix.TransformationMatrix);
			...
			foreach (BossRushOrb bossRushLock in activeBossRushLocks)
			{
				if (bossRushLock.isPlayingWarp)
				{
					if (bossRushLock.warpAnimationTimer > MAX_SUCC_ANIMATION)
					{
						...
						Main.spriteBatch.Begin(default, default, default, default, RasterizerState.CullNone, mapEffect, Main.GameViewMatrix.TransformationMatrix);
		...
		public static void DrawMap(SpriteBatch sb)
		{
			...
			foreach (BossRushOrb bossRushLock in activeBossRushLocks)
			{
				...
				sb.End();
				sb.Begin(default, BlendState.Additive, default, default, RasterizerState.CullNone, wobble, Main.GameViewMatrix.TransformationMatrix);
