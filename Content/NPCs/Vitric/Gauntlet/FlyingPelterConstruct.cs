			...
			if (attacking)
			{
				...
				if (bowFrame == 0)
				{
					...
					if (bowFrameCounter > timeToShoot)
					{
						...
						{
							var proj = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), bowPos + bowArmRotation.ToRotationVector2() * 5, bowArmRotation.ToRotationVector2() * 50, ModContent.ProjectileType<PelterConstructArrowLarge>(), NPC.damage, NPC.knockBackResist);
		...
		public override bool OnTileCollide(Vector2 oldVelocity)
		{
			if (Projectile.extraUpdates > 0)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					var proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, velocity, ModContent.ProjectileType<CoachGunEmber>(), 0, 0, 255);
					proj.friendly = false;
					proj.hostile = true;
					proj.scale = Main.rand.NextFloat(0.85f, 1.15f);
				}

				var proj2 = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<ConstructRing>(), Projectile.damage, 0);
