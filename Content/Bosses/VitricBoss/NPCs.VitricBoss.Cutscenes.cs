	...
	public sealed partial class VitricBoss : ModNPC
	{
		...
		private void SpawnAnimation() //The animation which plays when the boss is spawning
		{
			...
			if (checkSpecificTime(454))
			{
				if (Main.netMode != NetmodeID.Server)
					UILoader.GetUIState<TextCard>().Display(NPC.FullName, Main.rand.NextBool(10000) ? Language.GetTextValue("Mods.StarlightRiver.Custom.UI.MessageDisplay.CeriosMessage0") : Language.GetTextValue("Mods.StarlightRiver.Custom.UI.MessageDisplay.CeriosMessage1"), null, 310, 1.25f); //intro text
		...
		private void PhaseTransitionAnimation() //The animation that plays when the boss transitions from phase 1 to 2
		{
			...
			if (checkSpecificTime(140))
			{
				...
				if (IsInsideArena())
				{
					CameraSystem.DoPanAnimation(480, arena.Center());
				}
			}
			...
			if (GlobalTimer > 180 && GlobalTimer < 240)
			{
				...
				foreach (NPC crystal in crystals)
				{
					Vector2 start = (crystal.ModNPC as VitricBossCrystal).StartPos;
		...
		private void DeathAnimation() //The animation that plays when the boss dies
		{
			...
			if (GlobalTimer > 120 && Main.rand.NextBool(Math.Max(5, 60 - (int)GlobalTimer / 10)))
			{
