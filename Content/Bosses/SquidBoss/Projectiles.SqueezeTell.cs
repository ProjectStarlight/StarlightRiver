...
namespace StarlightRiver.Content.Bosses.SquidBoss
{
    class SqueezeTell : ModProjectile, IDrawAdditive
    {
		...
		{
            var arrow = ModContent.Request<Texture2D>(AssetDirectory.SquidBoss + "SqueezeTellArrow").Value;
            var glow = ModContent.Request<Texture2D>("StarlightRiver/Assets/GlowTrailNoEnd").Value;
            ...
            for(int k = 1; k < 9; k++)
			{
                ...
                for (int i = 0; i < 6; i++)
                {
                    var arrowColor = new Color(255, 100, 100) * (float)Math.Sin(prog * 6.28f * 4 - i) * (float)Math.Sin(prog * 3.14f);
